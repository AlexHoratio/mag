...
def validateInputParameters(hybrid) {
    ...
    if (!params.skip_binqc && !params.run_busco && !params.run_checkm && !params.run_checkm2) {
        log.warn('[nf-core/mag]: --skip_binqc is not specified, but no bin quality assessment tool is set to run! BINQC will be skipped.')
    }
    ...
    if (!params.skip_gtdbtk && params.skip_binqc) {
