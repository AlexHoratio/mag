...
With the parameter `--generate_bigmag_file` a local module will be triggered to generate one and unique file that contains the ouput from all of the bin-quality tools. Please note that it requires the parameters `--run_gunc` and `--run_checkm2` during the pipeline launching. GTDB-Tk2 and BUSCO default execution should be also enabled. The file `bigmag_summary.tsv`located at `GenomeBinning/BIgMAG` in the output directory can be used directly for the application [BIgMAG](https://github.com/jeffe107/BIgMAG), after the proper installation in their local environment. This is the only file they need to run the BIgMAG dashboard.
